package com.example.backservice;

import org.springframework.boot.SpringApplication;
import org.springframework.boot.autoconfigure.SpringBootApplication;

@SpringBootApplication
public class BackServiceApplication {

    public static void main(String[] args) {
        SpringApplication.run(BackServiceApplication.class, args);


        /**
         * Tasks:
         * <p>
         * 1. Make a readme to explain endpoints ( NOT DONE )
         * 2. Include True/False questions ( NOT DONE )
         * 3. Gradually increase difficulty if a user does good and decrease if user does bad ( NOT DONE )
         * 4. Introduce a points system, and upon hint usage half-credit is deducted ( NOT DONE )
         * 5. If any error occurs like while generating cause, AI has a rate limit pop up some error page ( NOT DONE )
         * 6. For Some reason when the app is run and tested on 1 site and then moves to the next on next, It does not extract from that
         * but from the previous website ( NOT DONE )
         * 7. Introduce it in quizzes, question where u have to type ( NOT DONE )
         * 8. Based on settings provide that type of questions ( NOT DONE )
         * 9. Based on time limit, time becomes 0 quiz is ended before u click finish, same for question ( NOT DONE )
         */
    }

}
